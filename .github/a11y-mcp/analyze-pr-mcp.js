...
function getResultsPath() {
  return process.env.A11Y_RESULTS_PATH || 'a11y-results.json';
}

async function importHybridAnalyzer() {
  const candidates = [
    path.join(__dirname, 'core', 'hybrid-analyzer.js'),
    path.join(__dirname, '..', 'src', 'core', 'hybrid-analyzer.js')
  ];

  for (const candidate of candidates) {
    if (fs.existsSync(candidate)) {
      const mod = await import(pathToFileURL(candidate).href);
      if (mod?.analyzeFileHybrid) return mod;
    }
  }

  throw new Error(`Could not find hybrid analyzer. Tried: ${candidates.join(', ')}`);
}
...
async function analyzePR() {
  try {
    ...
    if (relevantFiles.length === 0) {
      ...
      fs.writeFileSync(getResultsPath(), JSON.stringify(results, null, 2));
      return results;
    }
    ...
    try {
      ...
      for (const file of filesToCheck) {
        try {
          const { analyzeFileHybrid } = await importHybridAnalyzer();
